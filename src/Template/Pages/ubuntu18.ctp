...
<body>
  <div class="container-fluid">
    <div class="row  bg-info">
      <div class="col-4 ">
        <nav class="navbar navbar-expand right navbar-light left">
      ...
      <div class="col-8">
        ...
        <nav class="navbar right navbar-light menu2">
          <ul class="navbar-nav">
            <li class="nav-item">
        ...
        <nav class="navbar navbar-expand right navbar-light menu">
          <ul class="navbar-nav">
            <li class="nav-item">
              <a class="nav-link" href="">Home</a>
            </li>
      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <div class="col-8">
            ...
            <p class="">A "LAMP" stack is a group of open-source software that is typically installed together to enable a server to host dynamic websites and web apps. This term is actually an acronym which represents the Linux operating system, with the <strong>A</strong>pache web server. The site data is stored in a <strong>M</strong>ySQL database, and dynamic content is processed by <strong>P</strong>HP.</p>

            <p class="">In this guide, we will install a LAMP stack on an Ubuntu 18.04 server.</p>

            <h2>Prerequisites</h2>

            <p class="">In order to complete this tutorial, you will need to have an Ubuntu 18.04 server with a non-root sudo-enabled user account and a basic firewall. This can be configured using our <a href="https://www.digitalocean.com/community/tutorials/initial-server-setup-with-ubuntu-18-04"><u>initial server setup guide for Ubuntu 18.04</u></a>.</p>

            <h2>Step 1 — Installing Apache and Updating the Firewall</h2>

            <p class="">The Apache web server is among the most popular web servers in the world. It's well-documented and has been in wide use for much of the history of the web, which makes it a great default choice for hosting a website.</p>

            <p class=""> Install Apache using Ubuntu's package manager, <code>apt</code>:</p>

            <div class="cod">
              <p class=""><code>$ sudo apt update</code></p>
              <p class=""><code>$ sudo apt install apache2</code></p>
            </div>

            <p class="">Since this is a <code>sudo</code> command, these operations are executed with root privileges. It will ask you for your regular user's password to verify your intentions.</p>

            <p class="">Once you've entered your password, <code>apt</code> will tell you which packages it plans to install and how much extra disk space they'll take up. Press <code>Y</code> and hit <code>ENTER</code> to continue, and the installation will proceed.</p>

            <h3>Adjust the Firewall to Allow Web Traffic</h3>

            <p class="">Next, assuming that you have followed the initial server setup instructions and enabled the UFW firewall, make sure that your firewall allows HTTP and HTTPS traffic. You can check that UFW has an application profile for Apache like so:</p>

            <p class=""><code>$ sudo ufw app list</code></p>
            ...
            <p class="">If you look at the <code>Apache Full</code> profile, it should show that it enables traffic to ports <code>80</code> and <code>443</code>:</p>

            <p class=""><code>$ sudo ufw app info "Apache Full"</code></p>
            ...
            <p class="">Allow incoming HTTP and HTTPS traffic for this profile:</p>

            <p class=""><code>$ sudo ufw allow in "Apache Full"</code></p>

            <p class="">You can do a spot check right away to verify that everything went as planned by visiting your server's public IP address in your web browser (see the note under the next heading to find out what your public IP address is if you do not have this information already):</p>

            <p class=""><code>http://your_server_ip</code></p>
            ...
            <p class="">If you do not know what your server's public IP address is, there are a number of ways you can find it. Usually, this is the address you use to connect to your server through SSH.</p>

            <p class="">There are a few different ways to do this from the command line. First, you could use the iproute2 tools to get your IP address by typing this:</p>

            <p class=""><code>$ ip addr show eth0 | grep inet | awk '{ print $2; }' | sed 's/\/.*$//'</code></p>

            <p class="">This will give you two or three lines back. They are all correct addresses, but your computer may only be able to use one of them, so feel free to try each one.</p>

            <p class="">An alternative method is to use the <code>curl</code> utility to contact an outside party to tell you how it sees your server. This is done by asking a specific server what your IP address is:</p>

            <p class=""><code>$ sudo apt install curl<br>$ curl http://icanhazip.com</code></p>

            <p class="">Regardless of the method you use to get your IP address, type it into your web browser's address bar to view the default Apache page.</p>

            <h2>Step 2 — Installing MySQL</h2>

            <p class="">Now that you have your web server up and running, it is time to install MySQL. MySQL is a database management system. Basically, it will organize and provide access to databases where your site can store information.</p>

            <p class="">Again, use <code>apt</code> to acquire and install this software:</p>

            <p class=""><code>$ sudo apt install mysql-server</code></p>

            <p class="cod"><strong>Note</strong>: In this case, you do not have to run <code>sudo apt update</code> prior to the command. This is because you recently ran it in the commands above to install Apache. The package index on your computer should already be up-to-date.</p>

            <p class="">This command, too, will show you a list of the packages that will be installed, along with the amount of disk space they'll take up. Enter <code>Y</code> to continue.</p>

            <p class="">When the installation is complete, run a simple security script that comes pre-installed with MySQL which will remove some dangerous defaults and lock down access to your database system. Start the interactive script by running:</p>

            <p class=""><code>$ sudo mysql_secure_installation</code></p>

            <p class="">This will ask if you want to configure the <code>VALIDATE PASSWORD PLUGIN</code>.</p>

            <p class=""><strong>Note</strong>: Enabling this feature is something of a judgment call. If enabled, passwords which don't match the specified criteria will be rejected by MySQL with an error. This will cause issues if you use a weak password in conjunction with software which automatically configures MySQL user credentials, such as the Ubuntu packages for phpMyAdmin. It is safe to leave validation disabled, but you should always use strong, unique passwords for database credentials.</p>

            <p class="">Answer <code>Y</code> for yes, or anything else to continue without enabling.</p>

            <p class=""><code>VALIDATE PASSWORD PLUGIN can be used to test passwords and improve security. It checks the strength of password and allows the users to set only those passwords which are secure enough. Would you like to setup VALIDATE PASSWORD plugin?<br><br>Press y|Y for Yes, any other key for No:</code></p>

            <p class="">If you answer “yes”, you'll be asked to select a level of password validation. Keep in mind that if you enter <code>2</code> for the strongest level, you will receive errors when attempting to set any password which does not contain numbers, upper and lowercase letters, and special characters, or which is based on common dictionary words.</p>

            <p class=""><code>There are three levels of password validation policy:
              <br>
            ...
            <p class="">Regardless of whether you chose to set up the <code>VALIDATE PASSWORD PLUGIN</code>, your server will next ask you to select and confirm a password for the MySQL <strong>root</strong> user. This is an administrative account in MySQL that has increased privileges. Think of it as being similar to the <strong>root</strong> account for the server itself (although the one you are configuring now is a MySQL-specific account). Make sure this is a strong, unique password, and do not leave it blank.</p>

            <p class="">If you enabled password validation, you'll be shown the password strength for the root password you just entered and your server will ask if you want to change that password. If you are happy with your current password, enter <code>N</code> for "no" at the promp</p>

            <p class=""><code>Using existing password for root.
              <br>
              <br>Estimated strength of the password: 100
              <br>Change the password for root ? ((Press y|Y for Yes, any other key for No) : n
            </code></p>

            <p class="">For the rest of the questions, press <code>Y</code> and hit the <code>ENTER</code> key at each prompt. This will remove some anonymous users and the test database, disable remote root logins, and load these new rules so that MySQL immediately respects the changes you have made.</p>

            <p class="">Note that in Ubuntu systems running MySQL 5.7 (and later versions), the <strong>root</strong> MySQL user is set to authenticate using the <code>auth_socket</code> plugin by default rather than with a password. This allows for some greater security and usability in many cases, but it can also complicate things when you need to allow an external program (e.g., phpMyAdmin) to access the user.</p>

            <p class="">If you prefer to use a password when connecting to MySQL as root, you will need to switch its authentication method from <code>auth_socket</code> to <code>mysql_native_password</code>. To do this, open up the MySQL prompt from your terminal:</p>

            <p class=""><code>$ sudo mysql</code></p>

            <p class="">Next, check which authentication method each of your MySQL user accounts use with the following command:</p>
            ...
            <p class=""><code>mysql> ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY 'password'</code></p>

            <p class="">Then, run <code>FLUSH PRIVILEGES</code> which tells the server to reload the grant tables and put your new changes into effect:</p>

            <p class=""><code>mysql> FLUSH PRIVILEGES;</code></p>
            ...
            <p class=""><code>mysql> exit</code></p>

            <p class="">At this point, your database system is now set up and you can move on to installing PHP, the final component of the LAMP stack.</p>

            <h2>Step 3 — Installing PHP</h2>

            <p class="">PHP is the component of your setup that will process code to display dynamic content. It can run scripts, connect to your MySQL databases to get information, and hand the processed content over to your web server to display.</p>

            <p class="">Once again, leverage the <code>apt</code> system to install PHP. In addition, include some helper packages this time so that PHP code can run under the Apache server and talk to your MySQL database:</p>

            <p class=""><code>$ sudo apt install php libapache2-mod-php php-mysql</code></p>

            <p class="">This should install PHP without any problems. We'll test this in a moment.</p>

            <p class="">In most cases, you will want to modify the way that Apache serves files when a directory is requested. Currently, if a user requests a directory from the server, Apache will first look for a file called index.html. We want to tell the web server to prefer PHP files over others, so make Apache look for an index.php file first.</p>

            <p class="">To do this, type this command to open the <code>dir.conf</code> file in a text editor with root privileges:</p>

            <p class=""><code>$ sudo nano /etc/apache2/mods-enabled/dir.conf</code></p>

            <p class="">It will look like this</p>

            <p class=""><code>/etc/apache2/mods-enabled/dir.conf</code></p>

            <p class=""><code>&ltIfModule mod_dir.c&gt<br>DirectoryIndex index.html index.cgi index.pl index.php index.xhtml index.htm<br>&lt/IfModule&gt</code></p>

            <p class="">Move the PHP index file (highlighted above) to the first position after the DirectoryIndex specification, like this:</p>

            <p class=""><code>/etc/apache2/mods-enabled/dir.conf</code></p>

            <p class=""><code>&ltIfModule mod_dir.c&gt
