<main class="bd-masthead" id="content" role="main">
      <div class="container">
            ...
            <p> In this guide, we’ll get a <b>LAMP</b> stack installed on an Ubuntu 16.04 Droplet. Ubuntu will fulfill our first requirement: a Linux operating system.</p>
            ...
            <p>For our purposes, we can get started by typing these commands:</a>

                  <code>sudo apt-get update
                        sudo apt-get install apache2
                  </code>

                  <p>Since we are using a ‘sudo’ command, these operations get executed with root privileges. It will ask you for your regular user’s password to verify your intentions. Once you’ve entered your password, ‘apt’ will tell you which packages it plans to install and how much extra disk space they’ll take up. Press <b>Y</b> and hit <b>Enter</b> to continue, and the installation will proceed. </p>

                  <h3>Set Global ServerName to Suppress Syntax Warnings</h3>

                  <p>Next, we will add a single line to the <code class="highlighter-rouge">/etc/apache2/apache2.conf</code> file to suppress a warning message. While harmless, if you do not set <code class="highlighter-rouge">ServerName</code> globally, you will receive the following warning when checking your Apache configuration for syntax errors:
                        ...
                        <code class="highlighter-rouge">AH00558: apache2: Could not reliably determine the server's fully qualified domain name, using 127.0.1.1. Set the 'ServerName' directive globally to suppress this message
                              ...
                              <p> <strong>Note:</strong> In this case, you do not have to run ‘sudo apt-get update’ prior to the command. This is because we recently ran it in the commands above to install Apache. The package index on our computer should already be up-to-date. Again, you will be shown a list of the packages that will be installed, along with the amount of disk space they’ll take up. Enter <b>Y</b> to continue.
                                    ...
                                    When the installation is complete, we want to run a simple security script that will remove some dangerous defaults and lock down access to our database system a little bit. Start the interactive script by running:</p>

                                    <p> <code> mysql_secure_installation </code></p>

                                    <p> You will be asked to enter the password you set for the MySQL root account. Next, you will be asked if you want to configure the ‘VALIDATE PASSWORD PLUGIN’.
                                          ...
                                          <p> You’ll be asked to select a level of password validation. Keep in mind that if you enter <b>2</b>, for the strongest level, you will receive errors when attempting to set any password which does not contain numbers, upper and lowercase letters, and special characters, or which is based on common dictionary words.</p>
                                          ...
                                          <p>
                                                ...
                                                At this point, your database system is now set up and we can move on. </p>

                                                <h3>Step 3: Install PHP</h3>

                                                <p>PHP is the component of our setup that will process code to display dynamic content. It can run scripts, connect to our MySQL databases to get information, and hand the processed content over to our web server to display.
                                                ...
                                                <p>
                                                      ...
                                                      <b> /etc/apache2/mods-enabled/dir.conf </b> </p>
                                                      ...
                                                      <p>We want to move the PHP index file highlighted above to the first position after the ‘DirectoryIndex’ specification, like this:
                                                            ...
                                                            <p> When you are finished, save and close the file by pressing <b>Ctrl-X</b>. You’ll have to confirm the save by typing <b>Y</b> and then hit <b>Enter</b> to confirm the file save location.

                                                                  After this, we need to restart the Apache web server in order for our changes to be recognized. You can do this by typing this:</p>

                                                                  <p> <code> sudo systemctl restart apache2 </code> </p>

                                                                  <p> We can also check on the status of the ‘apache2’ service using ‘systemctl’: </p>

                                                                  <div class="highlighter-rouge"><div class="highlight"><pre class="highlight"><code>sudo systemctl status apache2
                                                                        **Sample Output**
                                                                  ...
                                                                  <p>To enhance the functionality of PHP, we can optionally install some additional modules.
                                                                        ...
                                                                        <p>If, after researching, you decide you would like to install a package, you can do so by using the ‘apt-get install’ command like we have been doing for our other software.

                                                                              If we decided that ‘php-cli’ is something that we need, we could type:<code> sudo apt-get install php-cli </code>If you want to install more than one module, you can do that by listing each one, separated by a space, following the ‘apt-get install’ command, like this:<code> sudo apt-get install package1 package2 ... </code> At this point, your <b>LAMP</b> stack is installed and configured. We should still test out our PHP though.</p>
                                                                              ...
                                                                              <p>
                                                                                    ...
                                                                                    The address you want to visit will be: http://your_server_IP_address/info.php The page that you come to should look something like this: </p>

                                                                                    <img src="/img/apache_image2.png" alt="Apache">

                                                                                    <p>This page basically gives you information about your server from the perspective of PHP. It is useful for debugging and to ensure that your settings are being applied correctly.
                                                                                          ...
                                                                                          You can always recreate this page if you need to access the information again later. </p>
