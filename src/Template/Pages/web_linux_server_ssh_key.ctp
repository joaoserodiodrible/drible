<main class="bd-masthead" id="content" role="main">
      <div class="container">
            ...
            <p>While there are a few different ways of logging into an SSH server, in this guide, we’ll focus on setting up SSH keys. SSH keys provide an easy, yet extremely secure way of logging into your server. For this reason, this is the method we recommend for all users. </p>
            ...
            <p>An overview of the flow is shown in this diagram:</p>

            <img src="/img/image_sshkeyauth.png" alt="SSH Key">

            <p>The diagram shows a laptop connecting to a server, but it could just as easily be one server connecting to another server</p>
            ...
            <h>How To Create SSH Keys</h2>
                  ...
                  <p>To do this, we can use a special utility called ssh-keygen, which is included with the standard OpenSSH suite of tools. By default, this will create a 2048 bit RSA key pair, which is fine for most uses.</p>
                  ...
                  <p>The utility will prompt you to select a location for the keys that will be generated. By default, the keys will be stored in the <code>~/.ssh</code> directory within your user’s home directory. The private key will be called <code>id_rsa</code> and the associated public key will be called <code>id_rsa.pub</code>.</p>

                  <p>Usually, it is best to stick with the default location at this stage. Doing so will allow your SSH client to automatically find your SSH keys when attempting to authenticate. If you would like to choose a non-standard path, type that in now, otherwise, press ENTER to accept the default.</p>
                  ...
                  <p>If you choose to overwrite the key on disk, you will not be able to authenticate using the previous key anymore. Be very careful when selecting yes, as this is a destructive process that cannot be reversed.</p>
                  ...
                  <p>Next, you will be prompted to enter a passphrase for the key. This is an optional passphrase that can be used to encrypt the private key file on disk.</p>
                  ...
                  <ul>
                        <li>
                              <p>The private SSH key (the part that can be passphrase protected), is never exposed on the network. The passphrase is only used to decrypt the key on the local machine. This means that network-based brute forcing will not be possible against the passphrase.</p>
                        </li>
                        <li>
                              <p>The private key is kept within a restricted directory. The SSH client will not recognize private keys that are not kept in restricted directories. The key itself must also have restricted permissions (read and write only available for the owner). This means that other users on the system cannot snoop.</p>
                        </li>
                        <li>
                              <p>Any attacker hoping to crack the private SSH key passphrase must already have access to the system. This means that they will already have access to your user account or the root account. If you are in this position, the passphrase can prevent the attacker from immediately logging into your other servers. This will hopefully give you time to create and implement a new SSH key pair and remove access from the compromised key.</p>
                        </li>
                  </ul>

                  <p>Since the private key is never exposed to the network and is protected through file permissions, this file should never be accessible to anyone other than you (and the root user). The passphrase serves as an additional layer of protection in case these conditions are compromised.</p>
                  ...
                  <p>You now have a public and private key that you can use to authenticate. The next step is to place the public key on your server so that you can use SSH key authentication to log in.</p>

                  <h2>How to Embed your Public Key when Creating your Server</h2>

                  <p>If you are starting up a new DigitalOcean server, you can automatically embed your SSH public key in your new server’s root account.</p>

                  <p>Towards the bottom of the Droplet creation page, there is an option to add SSH keys to your server:</p>

                  <img src="/img/image_addsshkeys1.png" alt="Add SSH Key">

                  <p>If you have already added a public key file to your DigitalOcean account, you will see it here as a selectable option (there are two existing keys in the example above: “Work key” and “Home key”). To embed an existing key, simply click on it and it will highlight. You can embed multiple keys on a single server:</p>

                  <img src="/img/image_addsshkeys2.png" alt="SSH Key">

                  <p>If you do not already have a public SSH key uploaded to your account, or if you would like to add a new key to your account, click on the “+ Add SSH Key” button. This will expand to a prompt:</p>

                  <img src="/img/image_addsshkeys3.png" alt="SSH Key">

                  <p>In the “SSH Key content” box, paste the content of your SSH public key. Assuming you generated your keys using the method above, you can obtain your public key contents on your local computer by typing:</p>

                  <p><code> cat ~/.ssh/id_rsa.pub </code></p>

                  <p><code> ssh-rsa  </code></p>

                  <p><code> AAAAB3NzaC1yc2EAAAADAQABAAABAQDNqqi1mHLnryb1FdbePrSZQdmXRZxGZbo0gTfglysq6KMNU NY2VhzmYN9JYW39yNtjhVxqfW6ewc+eHiL+IRRM1P5ecDAaL3 V0ou6ecSurU+t9DR4114mzNJ5SqNxMgiJzbXdhR+j55GjfXdk0FyzxM3a5qpVcGZEXiAzGzhHytUV51+YGnuLGaZ37nebh3UlYC+KJe v4MYIVww0tWmY+9GniRSQlgLLUQZ+FcBUjaqhwqVqsHe4F/w oW1IHe7mfm63GXyBavVc+llrEzRbMO111MogZUcoWDI9w7UIm8Z OTnhJsk7jhJzG2GpSXZHmly/a/buFaaFnmfZ4MY PkgJD username@example.com </code></p>

                  <p>Paste this value, in its entirety, into the larger box. In the “Comment (optional)” box, you can choose a label for the key. This will be displayed as the key name in the DigitalOcean interface:</p>

                  <img src="/img/image_addsshkeys4.png" alt="SSH Key">

                  <p>When you create your Droplet, the public SSH keys that you selected will be placed in the <code>~/.ssh/authorized_keys</code> file of the root user’s account. This will allow you to log into the server from the computer with your private key.</p>

                  <h2>How To Copy a Public Key to your Server</h2>

                  <p>If you already have a server available and did not embed keys upon creation, you can still upload your public key and use it to authenticate to your server.</p>

                  <p>The method you use depends largely on the tools you have available and the details of your current configuration. The following methods all yield the same end result. The easiest, most automated method is first and the ones that follow each require additional manual steps if you are unable to use the preceding methods.</p>

                  <h3>Copying your Public Key Using SSH-Copy-ID</h3>

                  <p>The easiest way to copy your public key to an existing server is to use a utility called <code>ssh-copy-id</code>. Because of its simplicity, this method is recommended if available.</p>

                  <p>The <code>ssh-copy-id</code> tool is included in the OpenSSH packages in many distributions, so you may have it available on your local system. For this method to work, you must already have password-based SSH access to your serve</p>

                  <p>To use the utility, you simply need to specify the remote host that you would like to connect to and the user account that you have password SSH access to. This is the account where your public SSH key will be copied.</p>
                  ...
                  <p>This just means that your local computer does not recognize the remote host. This will happen the first time you connect to a new host. Type “yes” and press ENTER to continue.</p>

                  <p>Next, the utility will scan your local account for the id_rsa.pub key that we created earlier. When it finds the key, it will prompt you for the password of the remote user’s account:</p>
                  ...
                  <p>At this point, your <code>id_rsa.pub</code> key has been uploaded to the remote account. You can continue onto the next section.</p>

                  <h3>Copying your Public Key Using SSH</h3>

                  <p>f you do not have <code>ssh-copy-id</code> available, but you have password-based SSH access to an account on your server, you can upload your keys using a conventional SSH method.</p>
                  ...
                  <p><code>cat ~/.ssh/id_rsa.pub | ssh username@remote_host "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys"</code></p>
                  ...
                  <p><code> username@111.111.11.111's password: </code> </p>

                  <p>After entering your password, the content of your <code>id_rsa.pub</code> key will be copied to the end of the <code>authorized_keys</code> file of the remote user’s account. Continue to the next section if this was successful.</p>
                  ...
                  <p>Now, you can create or modify the <code>authorized_keys</code> file within this directory. You can add the contents of your <code>id_rsa.pub</code> file to the end of the <code>authorized_keys</code> file, creating it if necessary, using this:</p>

                  <p><code>echo public_key_string >> ~/.ssh/authorized_keys</code></p>

                  <p>In the above command, substitute the public_key_string with the output from the <code>cat ~/.ssh/id_rsa.pub</code> command that you executed on your local system. It should start with <code>ssh-rsa AAAA...</code>.</p>

                  <p>If this works, you can move on to try to authenticate without a password.</p>

                  <h2>Authenticate to your Server Using SSH Keys</h2>

                  <p>If you have successfully completed one of the procedures above, you should be able to log into the remote host without the remote account’s password.</p>
                  ...
                  <p>This just means that your local computer does not recognize the remote host. Type “yes” and then press ENTER to continue.</p>

                  <p>If you did not supply a passphrase for your private key, you will be logged in immediately. If you supplied a passphrase for the private key when you created the key, you will be required to enter it now. Afterwards, a new shell session should be spawned for you with the account on the remote system.</p>
                  ...
                  <p>Before completing the steps in this section, make sure that you either have SSH key-based authentication configured for the root account on this server, or preferably, that you have SSH key-based authentication configured for an account on this server with <code>sudo</code> access. This step will lock down password-based logins, so ensuring that you have will still be able to get administrative access is essential.</p>

                  <p>Once the above conditions are true, log into your remote server with SSH keys, either as root or with an account with <code>sudo</code> privileges. Open the SSH daemon’s configuration file:</p>

                  <p><code> sudo nano /etc/ssh/sshd_config </code></p>

                  <p>Inside the file, search for a directive called <code>PasswordAuthentication</code>. This may be commented out. Uncomment the line and set the value to “no”. This will disable your ability to log in through SSH using account passwords:</p>

                  <p><code>PasswordAuthentication no</code></p>

                  <p>Save and close the file when you are finished. To actually implement the changes we just made, you must restart the service.</p>

                  <p>On Ubuntu or Debian machines, you can issue this command:</p>

                  <p><code>sudo service ssh restart</code></p>

                  <p>On CentOS/Fedora machines, the daemon is called <code>sshd</code>:</p>

                  <p><code> sudo service sshd restart </code></p>

                  <p>After completing this step, you’ve successfully transitioned your SSH daemon to only respond to SSH keys.</p>
