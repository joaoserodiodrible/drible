<main class="bd-masthead" id="content" role="main">
      <div class="container">
            ...
            <p>This tutorial will show you how to set up a firewall with UFW on Ubuntu 16.04.</p>
            ...
            <ul>
                  <li>One Ubuntu 16.04 server with a sudo non-root user, which you can set up by following Steps 1-3 in the <a href="https://www.digitalocean.com/community/tutorials/initial-server-setup-with-ubuntu-16-04">Initial Server Setup with Ubuntu 16.04 tutorial</a>.</li>
            </ul>

            <p>UFW is installed by default on Ubuntu. If it has been uninstalled for some reason, you can install it with <code>sudo apt-get install ufw</code>.</p>
            ...
            <p><code>sudo nano /etc/default/ufw</code>

                  <p>Then make sure the value of IPV6 is yes. It should look like this:</p>

                  <p><b>/etc/default/ufw excerpt</b></p>
                  ...
                  <p>This will create firewall rules that will allow all connections on port <code>22</code>, which is the port that the SSH daemon listens on by default. UFW knows what SSH and a number of other service names mean because they’re listed as services in the <code>/etc/services</code> file.</p>
                  ...
                  <p><code>sudo ufw allow 2222</code></p>
                  ...
                  <p><code> sudo ufw enable </code>
                        ...
                        <p>The firewall is now active. Feel free to run the <code>sudo ufw status verbose </code>command to see the rules that are set. The rest of this tutorial covers how to use UFW in more detail, like allowing or denying different kinds of connections.</p>

                        <h2>Step 4 — Allowing Other Connections</h2>

                        <p>At this point, you should allow all of the other connections that your server needs to respond to. The connections that you should allow depends your specific needs. Luckily, you already know how to write rules that allow connections based on a service name or port; we already did this for SSH on port <code>22</code>. You can also do this for:</p>

                        <ul>
                              <li>
                                    <p>HTTP on port 80, which is what unencrypted web servers use, using <code>sudo ufw allow http</code> or <code>sudo ufw allow 80</code></p>
                              </li>
                              <li>
                                    <p>HTTPS on port 443, which is what encrypted web servers use, using <code>sudo ufw allow https</code> or <code>sudo ufw allow 443</code></p>
                              </li>
                              <li>
                                    <p>FTP on port 21, which is used for unencrypted file transfers (which you probably shouldn’t use anyway), using <code>sudo ufw allow ftp</code> or <code>sudo ufw allow 21/tcp</code></p>
                              </li>
                        </ul>
                        ...
                        <p>You can specify port ranges with UFW. Some applications use multiple ports, instead of a single port.
                              ...
                              <p>When working with UFW, you can also specify IP addresses. For example, if you want to allow connections from a specific IP address, such as a work or home IP address of <code>15.15.15.51</code>, you need to specify <code>from</code>, then the IP address:</p>

                              <p> <code>sudo ufw allow from 15.15.15.51</code> </p>

                              <p>You can also specify a specific port that the IP address is allowed to connect to by adding <code>to any port</code> followed by the port number. For example, If you want to allow <code>15.15.15.51</code> to connect to port <code>22</code> (SSH), use this command:</p>

                              <p><code>sudo ufw allow from 15.15.15.51 to any port 22</code></p>
                              ...
                              <p><code>sudo ufw allow from 15.15.15.0/24</code></p>

                              <p>Likewise, you may also specify the destination port that the subnet <code>15.15.15.0/24</code> is allowed to connect to. Again, we’ll use port <code>22</code> (SSH) as an example:</p>

                              <p><code>sudo ufw allow from 15.15.15.0/24 to any port 22</code></p>
                              ...
                              <p>The highlighted output indicates the network interface names. They are typically named something like <code>eth0</code> or <code>eth1</code>.</p>
                              ...
                              <p><code>sudo ufw allow in on eth0 to any port 80</code></p>
                              ...
                              <p><code>sudo ufw allow in on eth1 to any port 3306</code></p>
                              ...
                              <p>However, sometimes you will want to deny specific connections based on the source IP address or subnet, perhaps because you know that your server is being attacked from there. Also, if you want change your default incoming policy to <b>allow</b> (which isn’t recommended in the interest of security), you would need to create <b>deny</b> rules for any services or IP addresses that you don’t want to allow connections for.</p>

                              <p>To write <b>deny</b> rules, you can use the commands described above, replacing <b>allow</b> with <b>deny</b></p>

                              <p>For example, to deny HTTP connections, you could use this command:</p>

                              <p><code>sudo ufw deny http</code></p>

                              <p>Or if you want to deny all connections from <code>15.15.15.51</code> you could use this command:</p>

                              <p><code>sudo ufw deny from 15.15.15.51</code></p>
                              ...
                              <p><code>sudo ufw delete 2</code></p>
                              ...
                              <p>The alternative to rule numbers is to specify the actual rule to delete. For example, if you want to remove the  <code>allow http</code> rule<, you could write it like this:
